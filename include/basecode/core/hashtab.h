...
namespace basecode::hashtab {
    ...
    Base_Type_Value insert(T& table,
                           const Key_Type& key,
    ...
    u0 reset(T& table) {
        if (table.flags) {
            std::memset(table.flags,
                        0,
                        hash_common::flag_words_for_capacity(table.capacity)
                        * sizeof(u64));
        }
        table.size = {};
    }
    ...
    u0 rehash(T& table, s32 new_capacity) {
        s32 idx = new_capacity == -1 ?
                  table.cap_idx : hash_common::find_nearest_prime_capacity(new_capacity);
        f32 lf;
        do {
        ...
        for (u32 i = 0; i < table.capacity; ++i) {
            ...
            u32 bucket_index = hash_common::range_reduction(table.hashes[i],
                                                            new_capacity);
    ...
    Base_Type_Value insert(T& table,
        ...
        if (hash_common::requires_rehash(table.size,
                                         table.capacity,
                                         table.load_factor)) {
            rehash(table);
        }
        ...
        u64 hash         = hash::hash64(key);
        u32 bucket_index = hash_common::range_reduction(hash, table.capacity);
        b8  found        = hash_common::find_free_bucket2(table.flags,
                                                          table.capacity,
                                                          bucket_index);
        if (found) {
