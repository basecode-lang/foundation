...
namespace basecode::assoc_array {
    ...
    u0 append(assoc_array_t<V>& array,
        ...
        if (array.size + 1 > array.capacity)
            grow_values(array);
        if (array.keys.size + (str.length + 1) > array.keys.capacity)
            grow_keys(array, str.length + 1);
    ...
    Value_Type* find(const assoc_array_t<V>& array,
                     const String_Concept auto& key) {
        for (u32 i = 0; i < array.size; ++i) {
            const auto& idx = array.index[i];
            if (key.length == idx.length
                &&  std::memcmp(array.keys.data + idx.offset,
                                key.data,
                                key.length) == 0) {
    ...
    u0 append(assoc_array_t<V>& array, const s8* str, const V& value) {
        ...
        if (array.size + 1 > array.capacity)
            grow_values(array);
        if (array.keys.size + length > array.keys.capacity)
            grow_keys(array, length);
