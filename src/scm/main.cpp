...
static char* make_import(void* alloc, const char* prefix, const char* mod, const char* suffix) {
    int preflen = strlen(prefix), modlen = strlen(mod);
    int len     = preflen + modlen + strlen(suffix);
    int suflen  = strlen(suffix) + (mod[0] == '(' ? 1 : 0);
    char* p, * impmod = (char*) sexp_malloc(alloc, len + 1);
...
static void check_nonull_arg(int c, const char* arg) {
    if (!arg) {
...
static void do_init_context(void* alloc, sexp* ctx, sexp* env, sexp_uint_t heap_size,
                            sexp_uint_t heap_max_size, sexp_sint_t fold_case) {
    *ctx = sexp_make_eval_context(alloc, NULL, NULL, NULL, heap_size, heap_max_size);
    if (!*ctx) {
...
#define init_context() if (! ctx) do {                                  \
      do_init_context(context::top()->alloc, &ctx, &env, heap_size, heap_max_size, fold_case); \
...
sexp run_main(int argc, char** argv) {
    ...
    for (i = 1; i < argc && argv[i][0] == '-'; i++) {
        switch ((c = argv[i][1])) {
            ...
            case 'm':arg    = ((argv[i][2] == '\0') ? argv[++i] : argv[i] + 2);
                ...
                impmod = make_import(sexp_context_alloc(ctx), prefix, arg, suffix);
                tmp    = check_exception(ctx,
                                         sexp_eval_string(ctx,
                                                          impmod,
                                                          -1,
                                                          (c == 'x' ? sexp_global(ctx, SEXP_G_META_ENV) : env)));
                sexp_free(sexp_context_alloc(ctx), impmod);
                if (c == 'x') {
                    sexp_set_parameter(ctx,
                                       sexp_global(ctx, SEXP_G_META_ENV),
                                       sexp_global(ctx, SEXP_G_INTERACTION_ENV_SYMBOL),
                                       tmp);
            ...
            case 'h':arg = ((argv[i][2] == '\0') ? argv[++i] : argv[i] + 2);
                ...
                break;
#if SEXP_USE_IMAGE_LOADING
            case 'i':arg = ((argv[i][2] == '\0') ? argv[++i] : argv[i] + 2);
                if (ctx) {
                    fprintf(stderr,
                            "-i <file>: image files must be loaded before other command-line options are specified: %s\n",
                            arg);
                    if (sexp_truep(sexp_global(ctx, SEXP_G_STRICT_P)))
                        exit_failure();
                }
                ctx = sexp_load_image(context::top()->alloc, arg, 0, heap_size, heap_max_size);
            ...
            case 'R':
                ...
                if (strcmp(main_module, "chibi.repl") == 0)
                    load_init(0);
                break;
            case 'r':main_symbol = argv[i][2] == '\0' ? "main" : argv[i] + 2;
                break;
            case 's':init_context();
                sexp_global(ctx, SEXP_G_STRICT_P) = SEXP_TRUE;
                handle_noarg();
                break;
            case 'T':init_context();
            ...
            case 't':mods_loaded = 1;
                ...
                *(char*) suffix = '\0';
                impmod            = make_import(sexp_context_alloc(ctx), sexp_trace_prefix, arg, sexp_trace_suffix);
    ...
    if (!quit || main_symbol != NULL) {
        ...
        if (i >= argc && main_symbol == NULL) {
            ...
            if (main_module != NULL) {
                impmod = make_import(sexp_context_alloc(ctx), "(load-module '(", main_module, "))");
            ...
            if (main_symbol) {
                ...
                if (sexp_procedurep(tmp)) {
                    ...
                    fprintf(stderr,
                            "couldn't find main binding: %s in %s\n",
                            main_symbol,
                            main_module ? main_module : argv[i]);
                }
            }
        }
    }
...
int main(int argc, const char** argv) {
    ...
    sexp_scheme_init();
    auto res = run_main(argc, (char**) argv);
    if (res == SEXP_FALSE) {
        exit_failure();
    } else {
        exit_success();
