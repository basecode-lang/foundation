...
namespace basecode::binfmt::ar {
    ...
    static status_t parse_headers(ar_t& ar, buf_crsr_t& c) {
        ...
        if (!OK(buf::cursor::read<u64>(c, thunk_u64.value)))
        ...
        while (CRSR_MORE(c)) {
            ...
            if (!OK(buf::cursor::read<header_t>(c, hdr)))
                break;
            ...
            if (!OK(buf::cursor::read<s8[2]>(c, marker)))
                break;
    ...
    static status_t parse_symbol_table(ar_t& ar, buf_crsr_t& c) {
        ...
        for (u32 i = 0; i < num_symbols; ++i) {
            ...
            if (!OK(buf::cursor::read<u32>(c, sym.offset)))
                return status_t::read_error;
