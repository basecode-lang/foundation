...
#include <basecode/core/hash/crc.h>

namespace basecode::hash::crc {
    u32 const g_crc32_table[256] = {
    ...
    u32 hash32(u0 const* data, usize len) {
        s32 remaining;
        u32 result = ~(u32) 0;
        auto const* c = (u8 const *)data;
        for (remaining = len; remaining--; c++)
            result = (result >> (u8) 8) ^ (g_crc32_table[(result ^ *c) & (u8) 0xff]);
        return ~result;
    }

    uint64_t hash64(void const* data, size_t len) {
        s64 remaining;
        u64 result = ~(u64) 0;
        auto const* c = (u8 const*) data;
        for (remaining = len; remaining--; c++)
            result = (result >> (u8) 8) ^ (g_crc64_table[(result ^ *c) & (u8) 0xff]);
        return ~result;
    }
}
