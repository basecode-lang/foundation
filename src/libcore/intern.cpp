...
namespace basecode::intern {
    ...
    static b8 find_key(const intern_t&, u64 hash, const str::slice_t& key, u32& bucket_index);
    ...
    static b8 find_key(const intern_t& pool, u64 hash, const str::slice_t& key, u32& bucket_index) {
        for (u32 i = bucket_index; i < pool.capacity; ++i) {
            if (pool.hashes[i] == 0) return false;
    ...
    static status_t rehash(intern_t& pool, u32 new_capacity) {
        ...
        const auto slices_size  = new_capacity * sizeof(str::slice_t);
        const auto buf_size     = ids_size + alignof(u64) + hashes_size + alignof(str::slice_t) + slices_size;
        ...
        auto slices = (str::slice_t*)   memory::system::align_forward(buf + ids_size + hashes_align + hashes_size, alignof(str::slice_t), slices_align);
