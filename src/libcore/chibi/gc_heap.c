...
static sexp_heap sexp_gc_packed_heap_make(void* alloc, size_t packed_size, size_t free_size) {
  ...
  sexp_heap heap = sexp_make_heap(alloc, sexp_heap_align(req_size), 0, 0);
  if (!heap) {
...
sexp sexp_gc_heap_pack(sexp ctx_src, sexp_uint_t heap_free_size) {
  ...
  state.heap = sexp_gc_packed_heap_make(sexp_context_alloc(ctx_src), state.sexps_size, heap_free_size);
  if (!state.heap) {
  ...
  heaps = sexp_malloc(sexp_context_alloc(ctx_src), sizeof(sexp_heap) * state.heaps_count);
  if (!heaps) {
    res = sexp_global(ctx_src, SEXP_G_OOM_ERROR);
    goto done; }
  ...
  state.index = 0;
  state.remap = sexp_malloc(sexp_context_alloc(ctx_src), sizeof(struct sexp_remap) * state.sexps_count);
  if (!state.remap) {
    res = sexp_global(ctx_src, SEXP_G_OOM_ERROR);
    goto done; }
  ...
  if (state.heap && res != SEXP_TRUE) { sexp_free_heap(sexp_context_alloc(ctx_src), state.heap); }
  if (state.remap) { sexp_free(sexp_context_alloc(ctx_src), state.remap); }
  if (heaps) { sexp_free(sexp_context_alloc(ctx_src), heaps); }
...
sexp sexp_save_image (sexp ctx_in, const char* filename) {
  ...
  if (fp) fclose(fp);
  if (heap) sexp_free_heap(sexp_context_alloc(ctx_in), heap);
        ...
        for (ls = sexp_global(ctx, SEXP_G_MODULE_PATH); sexp_pairp(ls); ls=sexp_cdr(ls)) {
          if (strstr(file_name, sexp_string_data(sexp_car(ls))) == file_name) {
            ...
            if (new_file_name) {
              ...
              sexp_free(sexp_context_alloc(ctx), new_file_name);
              if (sexp_dl_handle(dl))
                break;
            }
          }
...
sexp sexp_load_image (void* alloc, const char* filename, off_t offset, sexp_uint_t heap_free_size, sexp_uint_t heap_max_size) {
  ...
  state.heap = sexp_gc_packed_heap_make(alloc, header.size, heap_free_size);
  if (!state.heap) {
  ...
  */
  ...
  state.types = sexp_malloc(alloc, sizeof(sexp) * state.types_cnt);
  ...
  sexp_context_alloc(ctx) = alloc;
  res = ctx;
done:
  if (fp) fclose(fp);
  if (state.heap && !ctx) sexp_free(alloc, state.heap);
  if (state.types) sexp_free(alloc, state.types);
  return res;
}
