...
sexp sexp_make_thread (sexp ctx, sexp self, sexp_sint_t n, sexp thunk, sexp name) {
  ...
  res = sexp_make_eval_context(NULL, ctx, SEXP_FALSE, sexp_context_env(ctx), 0, 0);
...
sexp sexp_free_pollfds (sexp ctx, sexp self, sexp_sint_t n, sexp pollfds) {
  if (sexp_pollfds_fds(pollfds)) {
    sexp_free(sexp_context_alloc(ctx), sexp_pollfds_fds(pollfds));
...
static sexp sexp_insert_pollfd (sexp ctx, int fd, int events) {
  ...
  if (sexp_pollfds_num_fds(pollfds) == sexp_pollfds_max_fds(pollfds)) {
    ...
    sexp_free(sexp_context_alloc(ctx), pfd);
  }
