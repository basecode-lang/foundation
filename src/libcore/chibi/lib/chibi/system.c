...
sexp sexp_getgrnam_r_stub (sexp ctx, sexp self, sexp_sint_t n, sexp arg0, sexp arg2) {
  ...
  tmp1 = (struct group*) sexp_calloc(sexp_context_alloc(ctx), 1, 1 + sizeof(tmp1[0]));
  tmp4 = (struct group**) sexp_calloc(sexp_context_alloc(ctx), 1, 1 + sizeof(tmp4[0]));
...
sexp sexp_getpwnam_r_stub (sexp ctx, sexp self, sexp_sint_t n, sexp arg0, sexp arg2) {
  ...
  if (! sexp_stringp(arg2))
    return sexp_type_exception(ctx, self, SEXP_STRING, arg2);
  sexp_gc_preserve3(ctx, res, res1, res4);
  tmp1 = (struct passwd*) sexp_calloc(sexp_context_alloc(ctx), 1, 1 + sizeof(tmp1[0]));
  tmp4 = (struct passwd**) sexp_calloc(sexp_context_alloc(ctx), 1, 1 + sizeof(tmp4[0]));
