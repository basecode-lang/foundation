...
sexp sexp_make_eval_context (void* alloc, sexp ctx, sexp stack, sexp env, sexp_uint_t size, sexp_uint_t max_size) {
  sexp_gc_var1(res);
  alloc = ctx ? sexp_context_alloc(ctx) : alloc;
  res = sexp_make_context(alloc, ctx, size, max_size);
...
sexp sexp_make_child_context (sexp ctx, sexp lambda) {
  sexp res = sexp_make_eval_context(NULL,
                                    ctx,
...
static sexp analyze_lambda (sexp ctx, sexp x, int depth) {
  ...
  for (ls=sexp_cadr(x); sexp_pairp(ls); ls=sexp_cdr(ls)) {
    // XXX: need to change this to support default parameter values
...
sexp sexp_load_op (sexp ctx, sexp self, sexp_sint_t n, sexp source, sexp env) {
  ...
  if (sexp_exceptionp(in)) {
    ...
    ctx2 = sexp_make_eval_context(NULL, ctx, NULL, env, 0, 0);
...
#endif
#if SEXP_USE_BIGNUMS
    } else if (((sexp_sint_t)sexp_flonum_value(z) > SEXP_MAX_FIXNUM)
...
#define file_exists_p(path, buf) (! stat(path, buf))
  ...
  for ( ; sexp_pairp(ls); ls=sexp_cdr(ls)) {
    ...
    path = (char*) sexp_malloc(sexp_context_alloc(ctx), len);
    if (! path) return NULL;
    ...
    if (sexp_find_static_library(path) || file_exists_p(path, buf))
      return path;
    sexp_free(sexp_context_alloc(ctx), path);
  }
...
sexp sexp_find_module_file (sexp ctx, const char *file) {
  ...
  if (path) sexp_free(sexp_context_alloc(ctx), path);
  return res;
}
